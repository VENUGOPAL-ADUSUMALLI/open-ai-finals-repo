...
class JobPreference(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=Q(stipend_min__isnull=True)
                | Q(stipend_max__isnull=True)
    ...
    def clean(self):
        ...
        stipend_bounds = [self.stipend_min, self.stipend_max]
        ...
        if (
            ...
            raise ValidationError({'stipend_min': 'stipend_min must be less than or equal to stipend_max.'})
...
class MatchingRun(models.Model):
    ...
    candidate_profile_snapshot = models.JSONField(null=True, blank=True)
